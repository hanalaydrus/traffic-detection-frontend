...
export class TechScreenings extends Component {
  constructor() {
    super();
    this.state = {
            ...
            <div key={index}>
              <PageTitle title={row.label} titleStyle={{ marginTop: 40 }} />
                {
                  row.id === 1 &&
                    <div key={index} style={{ marginTop: 20 }}>
                      ...
                      <div className="float_left" style={{ width: 200 }}>
                        <h3>Score : {row.content.score}</h3>
                      </div>

                      <div className="float_left" style={{ width: 200 }}>
                        <h3>
                          Duration : {this.timeConverter(row.content.duration)}
                        </h3>
                      </div>
...

              {
                row.id !== 1 && row.id !== 4 && row.content.map( (row2, index2) => {
                  return (
                    <div key={index2}>
                      <h4 className="question" style={{ color: orange500 }}>
                        {row2.question}
                      </h4>
                      {
